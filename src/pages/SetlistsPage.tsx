...
import { Plus, FolderOpen, Calendar, Trash2, ListMusic, Pencil } from "lucide-react"
...
export function SetlistsPage() {
    ...
    const [editingList, setEditingList] = useState<Setlist | null>(null)
    ...
    const handleRename = () => {
        ...
        setlistStorage.update(editingList.id, { nome: editName })

        setEditingList(null)
        setEditName("")
        loadLists()
        addToast("Repertório renomeado!", "success")
    }
        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 pb-20 transition-colors">
            ...
            <main className="container mx-auto px-4 py-8 max-w-3xl">
                            ...
                            >
                                <CardHeader className="pb-3">
                                    <CardTitle className="text-lg flex justify-between items-center gap-2">
                                        {/* Adicionei truncate para o nome não 'atropelar' os botões se for muito longo */}
                                        <span className="truncate">{list.nome}</span>

                                        {/* Envolvemos os botões em uma div para eles ficarem juntos na direita */}
                                        <div className="flex items-center gap-1 shrink-0">
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    setEditingList(list);
                                                    setEditName(list.nome);
                                                }}
                                                className="text-zinc-400 hover:text-blue-500 transition-colors p-2 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-800"
                                                title="Renomear"
                                            >
                                            ...
                                            <button
                                                ...
                                                className="text-zinc-400 hover:text-red-500 transition-colors p-2 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-800"
                                                title="Apagar"
                                            >
                ...
                <Dialog open={!!editingList} onOpenChange={() => setEditingList(null)}>
                    <DialogContent>
                        <DialogHeader><DialogTitle>Renomear Repertório</DialogTitle></DialogHeader>
                        <Input
                            value={editName}
                            onChange={(e) => setEditName(e.target.value)}
                            placeholder="Novo nome do repertório"
                        />
                        <Button onClick={handleRename}>Salvar Alteração</Button>
                    </DialogContent>
                </Dialog>
            </main>
        </div>
